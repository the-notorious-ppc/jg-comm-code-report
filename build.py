"""
JustGloves Comm Code Report — build the static site from data/daily.csv.gz.
...
<title>JustGloves Comm Code Performance Report</title>
...
:root{
  ...
  --green:#6899CA; --green-deep:#4A73A0; --line:#E3E6E1;
...
.btn{font-family:inherit;font-size:13px;padding:7px 12px;border:1px solid #3A3A37;border-radius:6px;background:var(--ink-soft);color:#fff;cursor:pointer;white-space:nowrap}
...
<header>
  <div class="header-inner">
    <div class="brand">
      <img src="logo.svg" alt="JustGloves">
      <div>
...
function renderTrend(a,b){
  ...
  const datasets = [{
    label, data: keys.map(k => pick(cur[k])), borderColor:'#6899CA',
...
function exportCSV(){
  const all = rows();                       // every filtered row, not just the visible page
  const [a,b] = window._range;
  const cmp = document.getElementById('compare').value !== 'none';

  const head = ['Comm code','Cost (\u00a3)','% spend','Conv. value (\u00a3)','% value','ROAS',
                'Product impressions','Clicks','CTR %','CPC (\u00a3)','Conversions','CVR %'];
  ...
  for (const r of all){
    const row = [r.name, r.cost.toFixed(2), r.share.toFixed(1),
                 r.val.toFixed(2), r.vshare.toFixed(1), r.roas.toFixed(2),
                 r.impr, r.clk, r.ctr.toFixed(2), r.cpc.toFixed(2),
                 r.conv.toFixed(1), r.cvr.toFixed(2)];
    if (cmp) row.push(pc(r.dcost), pc(r.dval));
    lines.push(row.map(q).join(','));
  }

  // BOM so Excel reads it as UTF-8 and the pound signs survive.
  const blob = new Blob(['\ufeff' + lines.join('\r\n')], {type:'text/csv;charset=utf-8;'});
  const url = URL.createObjectURL(blob);
  const link = document.createElement('a');
  link.href = url;
  link.download = `justgloves-comm-codes_${D.dates[a]}_${D.dates[b]}.csv`;
...
function openModal(k){
  ...
  modalChart = new Chart(document.getElementById('mchart'), {
    ...
    data:{labels: keys.map(x => x.length===7 ? x : fmtD(x)), datasets:[
      ...
      {label:'Conv. value', data: keys.map(x=>map[x].val), borderColor:'#6899CA',
       backgroundColor:'rgba(63,165,53,.12)', fill:true, tension:.25,
