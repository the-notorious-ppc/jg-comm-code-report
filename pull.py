"""
JustGloves Comm Code Report — Google Ads data pull.
...
CUSTOMER_ID = "3881334759"          # JustGloves
...
UNMAPPED = "Unmapped"
FEED_SUFFIX = re.compile(r"(-sh|-twim)$", re.IGNORECASE)
...
def transform(raw: pd.DataFrame, lookup: pd.DataFrame) -> pd.DataFrame:
    ...
    # Anchored to the end only. A global replace would corrupt IDs that merely
    # contain "sh" or "twim" mid-string. -sh and -twim never co-occur on one SKU,
    # so a single pass is enough.
    stripped = df["item_id"].str.replace(FEED_SUFFIX, "", regex=True)
    n_stripped = (stripped != df["item_id"]).sum()
    print(f"  feed suffix (-sh/-twim) stripped from {n_stripped:,} rows")
    df["item_id"] = stripped
